...
#ifdef __MAC_OS_X_VERSION_MAX_ALLOWED
@implementation NSIndexPath (iOSLike)

+ (NSIndexPath *)indexPathForRow:(NSInteger)row inSection:(NSInteger)section
{
    const unsigned long indexes[] = { section, row };
...
@property (assign, nonatomic) RBQFetchedResultsChangeType changeType;
...
- (RBQSafeRealmObject *)safeObjectAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((NSUInteger)indexPath.section < cache.sections.count) {

        RBQSectionCacheObject *section = cache.sections[indexPath.section];

        if ((NSUInteger)indexPath.row < section.objects.count) {
...
- (NSInteger)numberOfRowsForSectionIndex:(NSInteger)index
{
    ...
    if ((NSUInteger)index < cache.sections.count) {
...
- (NSString *)titleForHeaderInSection:(NSInteger)section
{
    ...
    if (cache) {
        ...
        if ((NSUInteger)section < cache.sections.count) {
                             ...
                             state:(RBQStateObject *)state
{
    ...
    for (NSOrderedSet *sectionChanges in @[derivedChanges.deletedSectionChanges,
        ...
        for (RBQSectionChangeObject *sectionChange in sectionChanges) {
            ...
            if (sectionChange.changeType == RBQFetchedResultsChangeDelete) {
            ...
            else if (sectionChange.changeType == RBQFetchedResultsChangeInsert) {
    ...
    for (NSOrderedSet *objectChanges in @[derivedChanges.deletedObjectChanges,
        ...
        for (RBQObjectChangeObject *objectChange in objectChanges) {
            ...
            if (objectChange.changeType == RBQFetchedResultsChangeDelete) {
            ...
            else if (objectChange.changeType == RBQFetchedResultsChangeInsert) {
                // Insert the object
...
#ifdef DEBUG
                NSAssert((NSUInteger)objectChange.updatedIndexpath.row <= section.objects.count, @"Attemting to insert at index beyond bounds!");
#endif
            ...
            else if (objectChange.changeType == RBQFetchedResultsChangeMove) {
          ...
          sectionNameKeyPath:(NSString *)sectionNameKeyPath
{
     ...
     */
    if (((NSUInteger)controllerCache.fetchRequestHash != fetchRequest.hash) ||
                                ...
                                sectionChanges:(RBQSectionChangesObject *)sectionChanges
{
    ...
    for (RBQSectionCacheObject *section in sectionChanges.deletedCacheSections) {
            ...
            [self runOnMainThread:^() {
                [weakSelf.delegate controller:weakSelf
                             didChangeSection:sectionInfo
                                ...
                                forChangeType:RBQFetchedResultsChangeDelete];
            }];
        }
        ...
        sectionChange.changeType = RBQFetchedResultsChangeDelete;
    ...
    for (RBQSectionCacheObject *section in sectionChanges.insertedCacheSections) {
            ...
            [self runOnMainThread:^() {
                [weakSelf.delegate controller:weakSelf
                             didChangeSection:sectionInfo
                                ...
                                forChangeType:RBQFetchedResultsChangeInsert];
            }];
        }
        ...
        sectionChange.section = section;
        sectionChange.changeType = RBQFetchedResultsChangeInsert;
                                        ...
                                        state:(RBQStateObject *)state
{
    ...
    for (RBQObjectCacheObject *cacheObject in changeSets.cacheObjectsChangeSet) {
        ...
        if (! objectChange.updatedIndexpath &&
            ...
            objectChange.changeType = RBQFetchedResultsChangeDelete;
        ...
        // Inserted Objects
            ...
            objectChange.changeType = RBQFetchedResultsChangeInsert;
            ...
            if ([self.delegate respondsToSelector:
                ...
                [self runOnMainThread:^() {
                    [weakSelf.delegate controller:weakSelf
                                  didChangeObject:safeObject
                                      atIndexPath:objectChange.previousIndexPath
                                    forChangeType:RBQFetchedResultsChangeMove
                                     newIndexPath:objectChange.updatedIndexpath];
                }];
            }

            objectChange.changeType = RBQFetchedResultsChangeMove;
            ...
            if ([self.delegate respondsToSelector:
                ...
                [self runOnMainThread:^() {
                    [weakSelf.delegate controller:weakSelf
                                  didChangeObject:safeObject
                                      atIndexPath:objectChange.previousIndexPath
                                    forChangeType:RBQFetchedResultsChangeUpdate
                                     newIndexPath:objectChange.updatedIndexpath];
                }];
            }
        }
